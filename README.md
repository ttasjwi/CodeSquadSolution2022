...
### \[3.16 revision] isMovable 메서드가 더 이상 필요 없으므로 제거
1. 작업상황
...
### \[3.18 revision] Stage의 player 멤버변수 제거, 맵 상태를 Stack에 기록하는 시점 변경
1. 작업상황
   - Stage의 player 멤버변수 제거. 써보다보니 멤버로 쓰기엔 손해보는 구석이 많았다.
   - `recordCurrentTurn()` -> `recordBeforeTurn()` 메서드 명 변경
     - 메서드의 호출시기를, 맵 상태의 변화 직전으로 함. (실제 이동작업 수행 바로 직전)
2. 동작
